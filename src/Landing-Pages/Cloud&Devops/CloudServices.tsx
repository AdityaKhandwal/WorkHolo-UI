...
const Navbar = () => {
  ...
  const navLinks = [
    ...
    { name: 'Services', hasDropdown: true, active: true },
  ...
  return (
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white shadow-lg py-2' : 'bg-white py-4'}`}>
      {/* Top Bar */}
      {!isScrolled && (
        <div className="bg-black text-white text-[11px] py-1 px-4 md:px-12 flex justify-between items-center overflow-hidden whitespace-nowrap">
          <div className="flex gap-4 items-center">
            <span className="flex items-center gap-1"><Mail size={12} /> contact@mtouchlabs.com</span>
            <span className="flex items-center gap-1"><Phone size={12} /> +91 9390683154</span>
            <span className="flex items-center gap-1"><Phone size={12} /> +1 (551) 222-0070</span>
          </div>
          <div className="flex gap-4 items-center">
            <span className="text-accent-yellow font-bold">NASSCOM</span> SME Inspire Awards 2026 🏆
            <div className="hidden lg:flex gap-4 border-l border-white/20 pl-4">
              <span>Hyderabad</span>
              <span>Bangalore</span>
              <span>USA</span>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-12 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <div className="flex items-center">
            <div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center text-white font-bold text-xl">M</div>
            <div className="ml-2">
              <h1 className="text-black font-bold text-xl leading-none">mTouch Labs</h1>
              <p className="text-[8px] text-gray-500 uppercase tracking-widest">Certified ISO Company</p>
            </div>
          </div>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-8">
          {navLinks.map((link) => (
            <div key={link.name} className="flex items-center gap-1 cursor-pointer group">
              <span className={`text-sm font-medium ${link.active ? 'text-blue-600' : 'text-gray-700'} group-hover:text-blue-600 transition-colors`}>
                {link.name}
              </span>
              {link.hasDropdown && <ChevronDown size={14} className="text-gray-400 group-hover:text-blue-600" />}
            </div>
          ))}
          <button className="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2.5 rounded-full text-sm font-bold flex items-center gap-2 transition-all shadow-md">
      ...
      <AnimatePresence>
        {isMobileMenuOpen && (
          <motion.div 
            ...
            exit={{ opacity: 0, y: -20 }}
            className="lg:hidden absolute top-full left-0 w-full bg-white shadow-xl py-6 px-4 flex flex-col gap-4"
          >
            {navLinks.map((link) => (
              <div key={link.name} className="flex justify-between items-center py-2 border-b border-gray-100">
                <span className="text-gray-800 font-medium">{link.name}</span>
                {link.hasDropdown && <ChevronDown size={18} className="text-gray-400" />}
              </div>
            ))}
            <button className="bg-blue-500 text-white py-3 rounded-full font-bold mt-4">Request Quote</button>
          </motion.div>
        )}
      </AnimatePresence>
    </nav>
...
const Footer = () => {
  return (
    <footer className="bg-dark-bg text-white pt-24 pb-12 px-4 md:px-12">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-12 mb-24">
          <div>
            <h3 className="text-blue-400 font-bold text-xs uppercase tracking-widest mb-8 flex items-center gap-2">
              <div className="w-2 h-2 bg-blue-500 rounded-full" /> India
            </h3>
          ...
          <div>
            <h3 className="text-blue-400 font-bold text-xs uppercase tracking-widest mb-8 flex items-center gap-2">
              <div className="w-2 h-2 bg-blue-500 rounded-full" /> Americas
            </h3>
            <ul className="space-y-4 text-gray-400 text-sm">
              <li>United States</li>
              <li>Canada</li>
          ...
          <div>
            <h3 className="text-blue-400 font-bold text-xs uppercase tracking-widest mb-8 flex items-center gap-2">
              <div className="w-2 h-2 bg-blue-500 rounded-full" /> Europe
            </h3>
            <ul className="space-y-4 text-gray-400 text-sm">
          ...
          <div>
            <h3 className="text-blue-400 font-bold text-xs uppercase tracking-widest mb-8 flex items-center gap-2">
              <div className="w-2 h-2 bg-blue-500 rounded-full" /> Asia Pacific
            </h3>
            <ul className="space-y-4 text-gray-400 text-sm">
              <li>Singapore</li>
              <li>Australia</li>
          ...
          <div>
            <h3 className="text-blue-400 font-bold text-xs uppercase tracking-widest mb-8 flex items-center gap-2">
              <div className="w-2 h-2 bg-blue-500 rounded-full" /> Middle East
            </h3>
            <ul className="space-y-4 text-gray-400 text-sm">
              <li>UAE</li>
              <li>Dubai</li>
        ...
        <div className="grid lg:grid-cols-4 gap-12 border-t border-white/10 pt-16 mb-16">
          <div className="lg:col-span-1">
            <div className="flex items-center gap-2 mb-6">
              <div className="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center font-bold">M</div>
              <span className="text-xl font-bold">mTouch Labs</span>
            </div>
            <p className="text-gray-400 text-sm leading-relaxed mb-8">
              next-generation digital solutions that accelerate business transformation across industries.
            </p>
            <div className="space-y-4 text-sm text-gray-400">
              <div className="flex items-center gap-3"><Mail size={16} /> contact@mtouchlabs.com</div>
              <div className="flex items-center gap-3"><Phone size={16} /> +91 9390683154</div>
              <div className="flex items-center gap-3"><Phone size={16} /> +1 (551) 222-0070</div>
            </div>
            <div className="flex gap-4 mt-8">
              {[Linkedin, Twitter, Facebook, Instagram, Youtube].map((Icon, i) => (
                <div key={i} className="w-10 h-10 bg-white/5 rounded-full flex items-center justify-center hover:bg-blue-600 transition-colors cursor-pointer">
                  <Icon size={18} />
                </div>
              ))}
            </div>
          ...
          <div>
            <div className="bg-white/5 p-6 rounded-2xl border border-white/10">
              <div className="h-32 bg-gray-800 rounded-xl mb-4 flex items-center justify-center">
                <MapPin className="text-blue-500" />
              </div>
              <button className="w-full bg-blue-500 hover:bg-blue-600 py-3 rounded-full font-bold flex items-center justify-center gap-2 transition-all">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 pt-12 border-t border-white/10 text-xs text-gray-500">
