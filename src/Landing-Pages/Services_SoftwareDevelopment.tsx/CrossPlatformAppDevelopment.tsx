...
const Navbar = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-brand-dark/95 backdrop-blur-md py-3 shadow-lg' : 'bg-transparent py-6'}`}>
      {/* Top Bar */}
      {!isScrolled && (
        <div className="hidden lg:block border-b border-white/10 pb-4 mb-4">
          <div className="max-w-7xl mx-auto px-6 flex justify-between items-center text-xs font-medium opacity-80">
            <div className="flex items-center gap-6">
              <a href="mailto:contact@mtouchlabs.com" className="flex items-center gap-2 hover:text-brand-accent transition-colors">
                <Mail size={14} /> contact@mtouchlabs.com
              </a>
              <a href="tel:+919390683154" className="flex items-center gap-2 hover:text-brand-accent transition-colors">
                <Phone size={14} /> +91 9390683154
              </a>
              <a href="tel:+15512220070" className="flex items-center gap-2 hover:text-brand-accent transition-colors">
                <Phone size={14} /> +1 (551) 222-0070
              </a>
            </div>
            <div className="flex items-center gap-6">
              <span className="flex items-center gap-2">
                <span className="text-brand-accent">NASSCOM</span> SME Inspire Awards 2026 🏆
              </span>
      ...
      <nav className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <div className="w-10 h-10 bg-white rounded-full flex items-center justify-center overflow-hidden">
            <span className="text-brand-purple font-black text-xl italic">M</span>
          </div>
          <div className="flex flex-col leading-none">
            <span className="text-xl font-bold tracking-tight">mTouch Labs</span>
            <span className="text-[8px] uppercase tracking-[0.2em] opacity-60">Certified Company</span>
          </div>
        </div>
        ...
        <div className="hidden lg:flex items-center gap-8">
          {['Home', 'Services', 'Products', 'Resources', 'Portfolio', 'Careers', 'Contact Us'].map((item) => (
            <a 
              ...
              className="text-sm font-semibold hover:text-brand-accent transition-colors flex items-center gap-1 group"
            >
              {item}
              {(item === 'Services' || item === 'Products' || item === 'Resources') && (
        ...
        <button className="bg-blue-500 hover:bg-blue-600 text-white px-6 py-2.5 rounded-full text-sm font-bold flex items-center gap-2 transition-all transform hover:scale-105 active:scale-95 shadow-lg shadow-blue-500/20">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-8">
          {steps.map((step, idx) => (
            <div key={idx} className="flex flex-col items-center group">
              ...
              <span className="text-xs font-bold opacity-60 leading-tight">{step.title || (step as any).font}</span>
            </div>
          ))}
        </div>
      </div>
...
const Footer = () => {
  return (
    <footer className="bg-[#0B1120] pt-24 pb-12 text-white/60 text-sm">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid lg:grid-cols-4 gap-16 mb-20">
          <div className="col-span-1 lg:col-span-1">
            ...
            <p className="mb-8 leading-relaxed">
              A global technology partner engineering next-generation digital solutions that accelerate 
              business transformation across industries.
            </p>
            <div className="space-y-4 mb-8">
              <a href="mailto:contact@mtouchlabs.com" className="flex items-center gap-3 hover:text-white transition-colors">
                <Mail size={16} className="text-blue-400" /> contact@mtouchlabs.com
              </a>
              <a href="tel:+919390683154" className="flex items-center gap-3 hover:text-white transition-colors">
                <Phone size={16} className="text-blue-400" /> +91 9390683154
              </a>
              <a href="tel:+15512220070" className="flex items-center gap-3 hover:text-white transition-colors">
                <Phone size={16} className="text-blue-400" /> +1 (551) 222-0070
              </a>
            </div>
            <div className="flex gap-4">
              {[Linkedin, Twitter, Facebook, Instagram, Youtube].map((Icon, idx) => (
                <a key={idx} href="#" className="w-10 h-10 bg-white/5 rounded-xl flex items-center justify-center hover:bg-blue-500 hover:text-white transition-all">
                  <Icon size={18} />
                </a>
              ))}
            </div>
          ...
          <div>
            <h4 className="text-white font-black uppercase tracking-widest mb-8 text-xs">Company</h4>
            <ul className="space-y-4 font-bold">
              {['Home', 'Services', 'About Us', 'Careers', 'Contact Us'].map((link) => (
          ...
          <div>
            <h4 className="text-white font-black uppercase tracking-widest mb-8 text-xs">Services</h4>
            <ul className="space-y-4 font-bold">
              {[
          ...
          <div className="relative">
            <div className="rounded-3xl overflow-hidden h-64 bg-white/5 border border-white/10 group">
              <div className="absolute inset-0 flex items-center justify-center">
                <MapPin size={48} className="text-blue-500 animate-bounce" />
              </div>
              <div className="absolute bottom-4 left-4 right-4 bg-white/10 backdrop-blur-md p-4 rounded-2xl text-[10px] font-bold text-white">
                mTouch Labs - Software Development Company
              </div>
            </div>
            <button className="w-full mt-6 bg-blue-500 hover:bg-blue-600 text-white py-4 rounded-2xl font-black flex items-center justify-center gap-2 transition-all shadow-lg shadow-blue-500/20">
        ...
        <div className="border-t border-white/5 pt-20 mb-20">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-12">
            {[
              ...
              <div key={idx}>
                <h5 className="text-blue-400 font-black text-[10px] tracking-[0.2em] mb-6 flex items-center gap-2">
                  <div className="w-1.5 h-1.5 bg-blue-400 rounded-full" /> {loc.region}
                </h5>
        ...
        <div className="border-t border-white/5 pt-12 flex flex-col md:flex-row justify-between items-center gap-8 text-xs font-bold">
          <p>© 2026 <span className="text-white">mTouch Labs Pvt. Ltd.</span> All rights reserved.</p>
